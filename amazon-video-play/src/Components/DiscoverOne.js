...
import { NavLink } from 'react-router-dom';
 ...
 class DiscoverOne extends React.Component {
     constructor(){
         super();
         this.state={
             isLoading : true,
     ...
     FetchDiscover(){
         fetch(baseURL+`/discover/movie?api_key=${API_KEY}&with_genres=35`)
         .then((res) => res.json())
         .then((data) => {
             this.setState({
                 isLoading : false,
 ...
 <h2 className="block w-3/12 text-center text-white text-xl m-auto"> loading . . .</h2>                </div>
        ...
        return (
            <section className="-mt-20">
                ...
                <h2 className="text-2xl mx-2 text-white" id="movies">-</h2>
                    {
...
function EachArticle(props){
    ...
    return(
        <>
            <NavLink to={`/discoverone/${each.id}`}>
